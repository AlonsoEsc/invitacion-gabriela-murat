...
function emailHtml(invitation, response, language = "es", forCouple = false) {
  ...
  return `<!doctype html><html dir="${direction}"><body style="margin:0;background:#f7f1e7;color:#2f2020;font-family:Arial,sans-serif"><table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr><td align="center" style="padding:30px 15px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;background:#fff;border:1px solid #c6a15b"><tr><td style="background:#6f111d;color:#fff;padding:28px;text-align:center"><div style="color:#dfc98c;font-size:12px;letter-spacing:3px">GABRIELA &amp; MURAD</div><h1 style="margin:10px 0 0;font-family:Georgia,serif">${escapeHtml(copy.greeting)}</h1></td></tr><tr><td style="padding:30px"><h2 style="margin-top:0;color:#6f111d;font-family:Georgia,serif">${escapeHtml(invitation.displayName)}</h2><p><strong>${escapeHtml(status)}</strong></p><p>${escapeHtml(copy.date)}<br>${escapeHtml(copy.venue)}</p><p><strong>${escapeHtml(copy.guests)}:</strong><br>${names}</p>${response.message ? `<p><strong>${escapeHtml(copy.message)}:</strong><br>${escapeHtml(response.message)}</p>` : ""}<p style="margin-top:28px;color:#756865;font-size:12px">Código de confirmación: ${escapeHtml(response.confirmationCode)}</p></td></tr></table></td></tr></table></body></html>`;
}

async function send(resendApiKey, from, payload) {
...
export async function deliverRsvpEmails({ resendApiKey, from, coupleEmails, invitation, response }) {
  const recipients = coupleEmails.split(",").map((email) => email.trim()).filter(Boolean);
  const couplePromise = recipients.length
    ? send(resendApiKey, from, { to: recipients, subject: `RSVP: ${invitation.displayName} - ${response.status === "attending" ? "Asistirá" : "No asistirá"}`, html: emailHtml(invitation, response, "es", true) })
    : Promise.reject(new Error("couple-email-not-configured"));
  const guestPromise = send(resendApiKey, from, { to: [response.contactEmail], subject: (EVENT[response.language] || EVENT.es).subject, html: emailHtml(invitation, response, response.language) });
  ...
  return {
    overall: coupleSent && guestSent ? "sent" : coupleSent || guestSent ? "partial" : "failed",
    couple: coupleSent ? "sent" : "failed",
    guest: guestSent ? "sent" : "failed",
